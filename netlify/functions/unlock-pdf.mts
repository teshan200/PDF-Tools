...
export default async (req: Request) => {
  ...
  try {
    const body = await req.json() as { files: FilePayload[]; password?: string }
    const { files, password = '' } = body

    if (!files?.length) return Response.json({ error: 'Please provide a PDF file to unlock.' }, { status: 400 })
    ...
    const message = err?.response?.data?.error?.message || err?.message || 'Unlock failed. Check your password.'
    console.error('[unlock-pdf]', message, err?.response?.data || '')
    return Response.json({ error: message }, { status: 500 })
  }
}

export const config: Config = { path: '/api/unlock-pdf', method: ['POST'] }
